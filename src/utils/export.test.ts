...
describe('generateCsvContent', () => {
    it('should generate correct headers', () => {
        ...
        expect(result).toBe('Date,Habit Name,Status,Label,Note');
    });

    it('should format entries correctly', () => {
        const entries: ExportEntry[] = [
            { date: '2025-01-01', habitName: 'Run', status: 'Completed', label: '✓', note: 'Good run' },
            { date: '2025-01-02', habitName: 'Read', status: 'Failed', label: '✕', note: '' }
        ];

        const result = generateCsvContent(entries);
        const lines = result.split('\n');

        expect(lines[0]).toBe('Date,Habit Name,Status,Label,Note');
